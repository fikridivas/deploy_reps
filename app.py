...
#     data.append(int(no2))
    ...
    prediction=model.predict([[int(request.form['pm10']),int(request.form['pm25']),int(request.form['so2']),int(request.form['co']),int(request.form['o3']),int(request.form['no2'])]])
    output=prediction[0]
    # if prediction[0]==0:
    #     output=="Bagus"
    # elif prediction[0]==1:
